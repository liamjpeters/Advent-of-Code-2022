...
$Knots = [System.Collections.ArrayList]::new()

# Add 10 knots
foreach($Num in 0..9) {
    $Knots.Add(@(0,0)) | Out-Null
}
...
foreach($Instruction in $InputData) {
    ...
    for ($i = 0; $i -lt $Distance; $i++) {
        ...
        # then for each of the knots, work out if any moves are required
        for ($j = 0; $j -lt $Knots.Count - 1; $j++) {
            $KnotMove = IsMoveRequired $Knots[$j] $Knots[$j + 1]
            $Knots[$j + 1][0] += $KnotMove[0]
            $Knots[$j + 1][1] += $KnotMove[1]
        }
        # Check if the final knot (the tail) is in a different square
        # than last time. If so, record it
        $TailPosStr = "$($Knots[-1][0]),$($Knots[-1][1])"
