...
# going to play: A for Rock, B for Paper, and C for Scissors. The second column--" Suddenly, the Elf
...
# Convert the A,B,C and X,Y,Z of the strategy guide to names
...
foreach ($Game in $GameList) {
    ...
    $IPlayed = $RockPaperScissorsMap[$GameSplit[1]]
    $TheyPlayed = $RockPaperScissorsMap[$GameSplit[0]]
    $PointsTotal += $ScoreMap[$IPlayed][$TheyPlayed]
}
