...
# list. A monkey that starts a round with no items could end up inspecting and throwing many items 
# by the time its turn comes around. If a monkey is holding no items at the start of its turn, its 
# turn ends.
...
$Monkeys = . "$PSScriptRoot\Inputs.ps1"

function PrintMonkeys {
    param (
        [pscustomobject[]] $Monkeys,
        [hashtable] $Inspections
    )
    Write-Host "Monekys"
    Write-Host "================"
    foreach ($Monkey in $Monkeys) {
        Write-Host "Monkey $($Monkey.Number): $($Monkey.Items -join ',')"
    }
    Write-Host "Inspections"
    Write-Host "================"
    foreach ($Inspection in $Inspections.GetEnumerator()) {
        Write-Host "Monkey $($Inspection.Name): $($Inspection.Value)"
    }
}

$NumRounds = 20

# Set up the inspection counter for each monkey
$Inspected = @{}
for ($i = 0; $i -lt $Monkeys.Count; $i++) {
    $Inspected[$i] = 0
}

# Work out a number to cap possible values at.
# The 'divisible by' tests only care what the remainer is, not the actual value.
$CommonDivisor = 1
foreach ($Monkey in $Monkeys) {
...
foreach ($Round in 1..$NumRounds) {
    for ($i = 0; $i -lt $Monkeys.Count; $i++) {
        foreach ($Item in $Monkeys[$i].Items) {
            $Val = Invoke-Command -ScriptBlock $Monkeys[$i].Operation -ArgumentList $Item

            # Cap the number
            $Val = [Math]::Floor($Val / 3)

            if ($Val % $Monkeys[$i].test -eq 0) {
                $Monkeys[$Monkeys[$i].Truthy].Items += $Val
            } else {
                $Monkeys[$Monkeys[$i].Falsy].Items += $Val
            }
            $Inspected[$i]+=1
        }
        $Monkeys[$i].Items = @()
...
$Top2Inspected = $Inspected.GetEnumerator() | Select-Object -Expand 'Value' | 
                 Sort-Object -Descending | Select-Object -First 2 | ForEach-Object {
                    $_ -as [int32]
                 }

Write-Host "The level of monkey business after 20 rounds is '$($Top2Inspected[0] * $Top2Inspected[1])'"

#PrintMonkeys $Monkeys $Inspected