...
#  they'd like to know how many Calories are being carried by the Elf carrying the most Calories. In 
...
# Search for the index of the elf carrying the most calories
$HighestCaloriesSoFar = [int64]::MinValue
$HighestIndexSoFar = -1
for ($i = 0; $i -lt $ElvesArray.Count; $i++) {
    if ($ElvesArray[$i].Sum -gt $HighestCaloriesSoFar) {
        $HighestCaloriesSoFar = $ElvesArray[$i].Sum
        $HighestIndexSoFar = $i
    }
}

# Add 1 to go from 0-indexed to 1-indexed
Write-Host "Elf number $($HighestIndexSoFar+1) is carrying the most calories, $HighestCaloriesSoFar"