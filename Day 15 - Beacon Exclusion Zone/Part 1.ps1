...
# your pack contains a set of deployable sensors that you imagine were originally built to locate 
# lost Elves.
...
# exist at integer coordinates. Each sensor knows its own position and can determine the position of
# a beacon precisely; however, sensors can only lock on to the one beacon closest to the sensor as 
# measured by the Manhattan distance. (There is never a tie where two beacons are the same distance 
# to a sensor.)
...
# closest beacon to it is at 10,16.
...
# beacons that close or closer to that sensor. There could still be beacons that just happen to not 
# be the closest beacon to any sensor. Consider the sensor at 8,7:
...
$XUpper = [int64]::MinValue
$YUpper = [int64]::MinValue
$XLower = [int64]::MaxValue
$YLower = [int64]::MaxValue
...
foreach ($Item in $SensorAndBeaconData) {
    if ($Item -match $Pattern) {
        ...
        $Sensors += $Sensor
        $XUpper = [Math]::Max($Sensor.SensorX + $Sensor.MDist, $XUpper)
        $YUpper = [Math]::Max($Sensor.SensorY + $Sensor.MDist, $YUpper)
        $XLower = [Math]::Min($Sensor.SensorX - $Sensor.MDist, $XLower)
        $YLower = [Math]::Min($Sensor.SensorY - $Sensor.MDist, $YLower)
    } else {
...
Write-Host "Map is $Width x $Height"

$Y = 2000000
$XOffset = [Math]::Abs($XLower)
$Row = [Array]::CreateInstance([int64],$Width + $XOffset)
$Index = 1
$SensorCount = $Sensors.Count
foreach($Sensor in $Sensors) {
    Write-Progress -Activity "Mapping" -Status "Filling $Index" -PercentComplete ([Math]::Round(($Index / $SensorCount)*100))
    # Check if we are in range of this sensor
    $AbsDiff = [Math]::Abs($Sensor.SensorY - $Y)
    $Dist = $Sensor.MDist - $AbsDiff
    if ($Dist -gt 0) {
        foreach($X in ($Sensor.SensorX - $Dist)..($Sensor.SensorX + $Dist)) {
            $Row[$X + $XOffset] = 3
        }
    } elseif ($Dist -eq 0) {
        $Row[$Sensor.SensorX + $XOffset] = 3
    }
    $Index++
}
$Index = 1
foreach($Sensor in $Sensors) {
    Write-Progress -Activity "Mapping" -Status "Adding Sensors and Beacons $Index" -PercentComplete ([Math]::Round(($Index / $SensorCount)*100))
    if ($Sensor.SensorY -eq $Y) {
        $Row[$Xoffset + $Sensor.SensorX] = 1
    }
    if ($Sensor.BeaconY -eq $Y) {
        $Row[$Xoffset + $Sensor.BeaconX] = 2
    }
    $i ++
}
Write-Progress -Activity "Mapping" -Completed
$Row | Where-Object {$_ -eq 3} | Measure-Object