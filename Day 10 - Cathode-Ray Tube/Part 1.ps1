...
#   starting value, 1, plus all of the other addx values up to that point: 1 + 15 - 11 + 6 - 3 + 
#   5 - 1 - 8 + 13 + 4 = 21.)
...
class CPU {
    ...
    [int32] $ProgramClock = 0
    [int32] $RegisterX = 1

    [int32[]] $InterestingSignals = @()
    [int32[]] $InterestingValues = @()

    CPU([int32[]] $InterestingSignals) {
        $this.InterestingSignals = $InterestingSignals
        $this.PerformChecks()
    }

    [void] TickClock([int32] $Ticks) {
        foreach($Num in 1..$Ticks) {
            $this.ProgramClock++
            $this.PerformChecks()
        }
    }
    ...
    [void] ProcessInstructions ([string[]] $Instructions) {
        foreach($Instruction in $Instructions) {
            ...
            $Command = $SplitInst[0]
            $Arguments = $SplitInst[1] -as [int32]
            switch ($Command) {
                'addX' { 
                    $this.TickClock(2)
                    $this.RegisterX += $Arguments
                    break
                }
                'noop' {
                    $this.TickClock(1)
    ...
    hidden [void] PerformChecks () {
        if ($this.ProgramClock -in $this.InterestingSignals) {
            $this.InterestingValues += $this.RegisterX
        }
    }

    [int32[]] GetInterestingValues() {
...
function New-AoCCPU {
    ...
    [CPU]::new($InterestingSignals)
}

$Instructions = . "$PSScriptRoot\Inputs.ps1"

$InterestingSignals = @(20,60,100,140,180,220)

$CPU = New-AoCCPU $InterestingSignals

$CPU.ProcessInstructions($Instructions)

$InterestingValues = $CPU.GetInterestingValues()

$Sum = 0
for ($i = 0; $i -lt $InterestingValues.Count; $i++) {
    $Sum += $InterestingValues[$i] * $InterestingSignals[$i]
}

Write-Host "This sum is '$Sum'"