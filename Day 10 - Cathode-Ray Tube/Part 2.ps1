...
# sprite. (In this system, there is no such thing as "vertical position": if the sprite's horizontal
...
class CRT {
    ...
    [int32] $SpriteLocation = 0

    CRT() {}

    [void] SetSpriteLocation([int32] $Location) {
        $this.SpriteLocation = $Location
    }

    [void] TickClock() {
        $Pix = $this.CurrentPixel % $this.Width
        if ($Pix -in (($this.SpriteLocation - 1)..($this.SpriteLocation + 1))) {
            $this.Pixels += $this.Filled
        } else {
            $this.Pixels += $this.Empty
        }
        $this.CurrentPixel++
    }
    ...
    [void] Print() {
        for ($i = 0; $i -lt $this.Pixels.Count; $i++) {
            ...
            Write-Host $this.Pixels[$i] -NoNewline
        }
        Write-Host "`n" -NoNewline
    }
        ...
        foreach($Instruction in $Instructions) {
            ...
            switch ($Command) {
                'addX' { 
                    ...
                    $this.CRT.SetSpriteLocation($this.RegisterX)
                    break
                }
                'noop' {
                    $this.TickClock(1)
