...
# effort, the Elves pair up and make a big list of the section assignments for each 
...
function CheckRangesForTotalEncapsulation {
    # Returns $true if one of the assignements is full contained in the other
    # including the case where the ranges are identical
    param (
    ...
    # Compare the two assignment arrays. This pulls back a 'sideindicator' to show
    # which object (reference '<=', or difference '=>') each number appears in where
    # it does not appear in the other.
    # We take the unique sum so we, at most, have 1 of each symbol (if there are unique numbers in
    # each list). If we have just 1 sideindicator (either '<=' or '=>') then we know that
    # one list must fully include the other.
    # If we have 0 sideindicators, we know both ranges were identical.
    Compare-Object $Assignment1Expanded $Assignment2Expanded | 
        Select-Object -ExpandProperty 'SideIndicator' -Unique |
        Measure-Object |
...
foreach ($SectionAssignment in $SectionAssignments) {
    if (CheckRangesForTotalEncapsulation $SectionAssignment) {
        $RunningTotal++
    }
}

Write-Host "There are $RunningTotal assignment pairs where one range fully contains the other"